...
describe Lita::Adapters::Slack do
  ...
  describe "#send_messages" do
    it "sends JSON payload via HTTP POST to Slack channel" do
      user = double("Lita::User", name: nil, metadata: {})
    ...
    it "sends message with mention if user info is provided" do
      user = double("Lita::User", id: "UM3NT10N", name: nil, metadata: {})
      target = double("Lita::Source", room: "CR00M1D", user: user)
      text = "<@#{user.id}> Hello!"
    ...
    it "proceeds but logs WARN when directed to an user without channel(room) info" do
      ...
      text = "<@#{user.id}> Hello!"
      payload = { username: Lita.config.adapter.username, text: text }
    ...
    it "adds a user icon and username to the payload when provided" do
      user = double("Lita::User", id: "UM3NT10N", name: "John", metadata: { icon_url: "http://example.com/icon.png" })
      target = double("Lita::Source", user: user, room: "CR00M1D")
      text = "<@#{user.id}> Hello!"

      payload = { channel: target.room, username: "John", text: text, icon_url: "http://example.com/icon.png"}
      expect(subject).to receive(:http_post).with(payload)
      subject.send_messages(target, ["Hello!"])
    end
  end
